//
// black_one_normal.jpgでのみ上手くグループ化でき，目のグループからサイコロの個数を数えることが可能．
// 基本的にCUIとの対話形式で処理を行う
//
...
namespace face{
      ...
      )
    {
        ...
        if(sources.size() > 1) grouping(groups,sources);
...
namespace dice{
    // 0<= angle < 180
    std::pair<float,float> circle_center_angle_distance(const cv::Point2f& center_1, const cv::Point2f& center_2)
    {
        //逆三角関数と余弦定理etc
        const double a = center_2.y - center_1.y;
        const double b = calc_distance(center_1,center_2);
        const double c = center_2.x - center_1.x;

        //分母が0になる場合を想定
        if(c == 0.0) return std::make_pair<float,float>(90.0, b);

        double angle = acos((b*b + c*c - a*a) / (2 * b * c)) * 180.0 / PI;
        if(angle == 180.0) angle = 0.0;
        return std::make_pair<float,float>(angle, b);
    }

    bool is_circle_on_rect(const std::pair<cv::Point2f,float>& circle, const cv::Rect& rect, const float range = 60)
    {
        const cv::Point2f rect_center(rect.x + (rect.width/2.0), rect.y + (rect.height/2.0));
        if(circle.first.y > rect_center.y) return false;

        const float half_range = range / 2.0;
        float angle, distance;
        std::tie(angle, distance) = circle_center_angle_distance(rect_center,circle.first);

        if(((angle > 90-half_range) && (angle < 90+half_range)) || (distance < ((circle.second + rect.height / 2.0) / 2.0 * 3.0))) return true;
        else return false;
    }

    //四隅でなく，上下左右．適当
    void circle_to_four_point(std::vector<cv::Point2f>& dst, const std::pair<cv::Point2f,float>& circle)
    {
        const cv::Point2f& center = circle.first;
        const float& radius = circle.second;
    ...
    void grouping(std::vector<std::pair<cv::Point2f,float>>& src_circles,std::vector<std::vector<std::pair<cv::Point2f,float>>>& dst_groups)
    {
        for(size_t i=0; i<src_circles.size(); ++i)
        {
            for(size_t j=i+1; j<src_circles.size(); ) // not Incl
            {
                const auto info = circle_center_angle_distance(src_circles[i].first, src_circles[j].first);
                const float average_radius = (src_circles[i].second + src_circles[j].second) / 2.0;
                //const cv::Rect rect = cv::boundingRect(src_circles[j]);

                // 下となる円の重要ポイントを入れる
                std::vector<cv::Point2f> points;
                circle_to_four_point(points,src_circles[i]);
                if(((info.first < 30) || (info.first > 150)) && (info.second < average_radius * 3.0))
                {
                    circle_to_four_point(points,src_circles[j]);
                }

                const cv::Rect bottom_rect = cv::boundingRect(points);
                if(points.size() == 4)
                {
                    // i円の上にj円が乗ってるかどうか
                    if( is_circle_on_rect(src_circles[j], bottom_rect) )
                    {
                        std::vector<std::pair<cv::Point2f,float>> group;
                        group.push_back(src_circles[i]);
                        group.push_back(src_circles[j]);
                        src_circles.erase(src_circles.begin() + j);
                        src_circles.erase(src_circles.begin() + i);
                        dst_groups.push_back(std::move(group));
                ...
                else if(points.size() == 8)
                {
                    // i円とj円の上にk点が乗るか
                    bool match = false;
                    for(size_t k=0; k<src_circles.size(); ++k)
                    {
                        if(k==i || k==j) continue;

                        if( match = is_circle_on_rect(src_circles[k], bottom_rect) )
                        {
                            size_t min, mid, max;
                            if(k < i) min = k, mid = i, max = j;
                            else if(k < j) min = i, mid = k, max = j;
                            else min = i, mid = j, max = k;
                                
                            std::vector<std::pair<cv::Point2f,float>> group;
                            group.push_back(src_circles[min]);
                            group.push_back(src_circles[mid]);
                            group.push_back(src_circles[max]);
                            src_circles.erase(src_circles.begin() + max);
                            src_circles.erase(src_circles.begin() + mid);
                            src_circles.erase(src_circles.begin() + min);
                            dst_groups.push_back(std::move(group));
...
int main()
{
    ...
    cv::Mat image = cv::imread("black_one_normal.jpg",1),hsv;
    ...
    std::vector<std::vector<cv::RotatedRect>> groups;
    face::grouping(groups,black_boxes);
    face::merge(groups);
    ...
    std::cout << "Found: " << groups.size() << std::endl;
    std::cout << "--End--\n" << std::endl;
    ...
    //
    temp = image.clone();
    std::vector<std::pair<cv::Point2f,float>> circle_group;
    for(size_t i = 0; i < groups.size(); ++i)
    {
        std::vector<cv::Point2f> points;
        ...
        cv::minEnclosingCircle(points,circle.first,circle.second);
        circle_group.push_back(std::move(circle));
    }
    for(auto it=circle_group.cbegin(); it!=circle_group.cend(); ++it)
    {
        cv::circle(temp,it->first,it->second,colors[0],2);
    }
    ...
    //ダイス型にグループ分け    
    std::cout << "--Start \"Grouping\"--" << std::endl;
    std::vector<std::vector<std::pair<cv::Point2f,float>>> dice_grouped;
    std::sort(circle_group.begin(),circle_group.end(),
    [](const std::pair<cv::Point2f,float>& lhs,const std::pair<cv::Point2f,float>& rhs)->bool{
        return (lhs.first.y == rhs.first.y) ? (lhs.first.x < rhs.first.x) : (lhs.first.y < rhs.first.y);
    });
    dice::grouping(circle_group,dice_grouped);
    std::cout << "Found: " << dice_grouped.size() << std::endl;
    std::cout << "--End--\n" << std::endl;

    temp = image.clone();
    for(size_t i=0; i<dice_grouped.size(); ++i)
    {
        std::vector<cv::Point2f> points;
        for(auto it = dice_grouped[i].cbegin(), end = dice_grouped[i].cend(); it != end; ++it)
        {
            dice::circle_to_four_point(points,*it);
        }

        std::pair<cv::Point2f,float> circle;
        cv::minEnclosingCircle(points, circle.first, circle.second);
        cv::circle(temp, circle.first, circle.second, colors[0], 2);
    }
    cv::namedWindow("diced");
    cv::imshow("diced",temp);
    {
        char inputkey;
        while(inputkey = cv::waitKey(0), inputkey != 'q'); //qが入力されるまで待機
    }
    cv::destroyWindow("diced");
