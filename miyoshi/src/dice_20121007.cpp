...
const char* WINDOW_COUNT="Counted Dice: Wants to add dice, Select dice then Press the size, to finish, Press \'n\'";
...
void CountDice(const cv::Mat& image,std::map<int,Dice>& top,std::map<int,Dice>& side);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	red_area(red,input);
	ioGeteye(input,black,red,blackeye,redeye,true);
	ioGetdice(input,top,side,blackeye,redeye,true);
	CountDice(input,top,side);
	cv::destroyAllWindows();
	std::cout<<"\nPushing any keys leads to exit..."<<std::endl;
	std::cin.get();
...
void onMouse_crop(int event,int x,int y,int flag,void* param)
{
	...
	else if(event == CV_EVENT_LBUTTONUP)
	{
		...
		std::pair<cv::Mat*,cv::Rect*>* data=reinterpret_cast<std::pair<cv::Mat*,cv::Rect*>*>(param);
...
void crop(cv::Mat& image)
{
	std::pair<cv::Mat*,cv::Rect*> senddata;
	senddata.first=&image;
	senddata.second=new cv::Rect(0,0,image.cols,image.rows);
	...
	cv::Mat cropped(image,*senddata.second);
	image=cropped.clone();
	cv::destroyWindow(WINDOW_CROP);
}
...
{
	char flag;
	static int dist=30,r_h=14,r_aspect=14;
...
void getdice(const cv::Mat& image,std::map<int,Dice>& top,std::map<int,Dice>& side,std::map<int,cv::Size>& eyes,double dist,double r_h,double r_aspect)
{
	...
	for(std::map<int,cv::Size>::iterator i=eyes.begin();i!=eyes.end();i++)
	{
		...
		if(visited.find((*i).first)==visited.end())
		{
			...
			temp.push_back(*i);
			coordinate.push_back(p2cvp((*i).first,temp_img.cols));

			//近傍判定(他の目が(基準目の高さ+幅)/2のdist倍以内にあるか?)

			for(std::map<int,cv::Size>::iterator ite=eyes.begin();ite!=eyes.end();ite++)
			{
				if(near_ellipse((*ite).first,(*i).first,h*dist,w*dist,image.cols))
				{
					//大きさ判定(高さの比がr_h倍以内･縦横比r_aspect倍以内,目が小さいときは補正を)
					double rh=(*i).second.height<image.cols/160?r_h*2:r_h;
					double raspect=(*i).second.height<image.cols/160?r_aspect*2:r_aspect;
		...
		if(temp.size()>1)
		{
			...
			else
			{
				...
				cv::ellipse(temp_img,box.center,cv::Size(w,h),0,0,360,cv::Scalar(0,255,0),1,CV_AA);
			}
		}
	}
...
void redeye2dice(std::map<int,Dice>& top,std::map<int,Dice>& side,std::map<int,cv::Size>& redeye)
{
	...
	{
		//1の面の大きさは目の大きさの2倍(実測)
		Dice d((*it).second.height*2,(*it).second.width*2,((*it).second.height+(*it).second.width)/2,1,(double)(*it).second.height/(*it).second.width,std::max((*it).second.height,(*it).second.width));
...
void CountDice(const cv::Mat& image,std::map<int,Dice>& top,std::map<int,Dice>& side)
{
	const int band=10;//number of the band
...
void count(const cv::Mat& image,cv::Mat& counted_img,std::multimap<char,int>& dicesize,const std::map<int,Dice>& dice,int top)
{
	...
	for(std::map<int,Dice>::const_iterator it=dice.begin();it!=dice.end();it++)
	{
		...
		{
		case 'l':
			if((*it).second.compsize>=(*pos).second.compsize*0.75)
			{
				size='l';
			}
			else if((*it).second.compsize>=(*pos).second.compsize*0.5)
			{
				size='m';
			}
			else if((*it).second.compsize>=(*pos).second.compsize*0.3)
			{
				size='s';
			}
			break;
		case 'm':
			if((*it).second.compsize>=(*pos).second.compsize*0.8)
			{
				size='m';
			}
		...
		cv::ellipse(counted_img,p2cvp((*it).first,image.cols),cv::Size((*it).second.w,(*it).second.h),0,0,360,color,1,CV_AA);
	}
}
...
void remove(std::map<int,Dice>& top,std::map<int,Dice>& side,int cols)
{
	//上に側面がない上面並びにその直ぐ下の側面を削除
	//getdiceの後処理
	
	//下から辿る
	for(std::map<int,Dice>::reverse_iterator rit=top.rbegin();rit!=top.rend();rit++)
	{
		for(std::map<int,Dice>::iterator its=side.begin();its!=side.end();its++)
		{
			//上に側面のない上面には消えて頂く
			if(distant((*rit).first,(*its).first,(*rit).second.h*2,cols)&&(*rit).first/cols-(*its).first/cols>(*rit).second.h/2)
			{
				//消える上面の直ぐ下の側面にも消えて頂く
				for(std::map<int,Dice>::iterator itsd=its;itsd!=side.end();itsd++)
				{
					if(distant((*rit).first,(*itsd).first,(*rit).second.h*2,cols)&&(*itsd).first/cols-(*rit).first/cols>(*rit).second.h/2)
					{
						side.erase(itsd);
					}
				}
