...
void sharp(const cv::Mat src, cv::Mat& dst, float k = 2.0f)
{
	float kernelData[] = {
...
int main(int argc, char** argv)
{
	if(argc>1){
		...
		for(int num=1; num<argc; ++num){
			...
			// 赤黒別前処理
			cv::Mat dst[2];
			cv::medianBlur(src, dst[0], 13);
			normalise(dst[0], dst[0]);
			sharp(src, dst[1], 3.2f);

			// 赤と黒を取り出す
			cv::Mat red,black,hsv;
			cv::cvtColor(dst[0], hsv, CV_BGR2HSV);
			red_area(red,hsv);
			black_area(black,dst[1]);
